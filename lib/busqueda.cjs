 ...
 *
 * POR QUÉ EXISTE: el motor de búsqueda es de la plataforma (ADR 0006), pero su
 ...
 *
 * EL FUZZY SE QUEDA PRENDIDO, y conviene dejar escrito por qué para no volver
 * a intentarlo. El tema trae `fuzzyMatchingDistance: 1` desde su 0.49.0:
 * cuando las consultas exactas no llenan los resultados, completa con
 * términos a distancia de edición 1. Suena a ruido y se probó apagarlo,
 * medido contra el índice PUBLICADO (no uno de prueba), con el propio cliente
 * del tema:
 *
 *   - en 12 consultas bien escritas ('costo de merc', 'conciliacion
 *     bancaria', 'facturas de clientes', 'notas de credito', 'sircreb'...)
 *     el resultado es IDÉNTICO con 1 y con 0. No hay cola que limpiar: los
 *     lugares se llenan con coincidencias exactas antes de que corra una
 *     consulta difusa.
 *   - en 5 de 6 consultas con un tipeo, apagarlo deja CERO resultados donde
 *     hoy encuentra lo correcto: `persepciones` da 8 aciertos sobre
 *     `percepcion`, `facturacon` 8 sobre `facturacion`, y `conciliacion
 *     bancria`, `mercaderia vendia` y `nota de credto` también encuentran.
 *
 * O sea: beneficio cero y regresión real para quien escribe rápido. Si alguna
 * vez molesta, lo que hay que atacar es el orden de los resultados, no el
 * fuzzy: el ruido que se le atribuye sale de que el tema nunca compara puntaje
 * entre las versiones de la consulta.
 */
