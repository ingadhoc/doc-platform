/**
 * Carga del índice del agente y motor de búsqueda. VERSIÓN UNIFICADA
 * (best-of-three de oba-docs / odumbo-docs / adhoc-docs).
 *
 * TODO ESTE MÓDULO CORRE UNA SOLA VEZ POR INSTANCIA. Vercel corre las
 * funciones con Fluid Compute (default en proyectos nuevos — hay que
 * verificar el toggle en cada proyecto): el scope de módulo persiste entre
 * invocaciones, así que leer el JSON y construir el índice de MiniSearch acá
 * se paga una vez por instancia, no por request.
 *
 * De dónde sale el archivo: lo emite el preprocesador (`tools/build.mjs`,
 * target `agente`) en `api/_generated/index.json`, por audiencia, en cada
 * build. Vive adentro de `api/` a propósito para que quede al lado de la
 * función; como es `.json`, Vercel no lo convierte en un endpoint.
 *
 * Cómo entra al bundle de la función: `fs.readFileSync` con una ruta armada
 * en runtime NO la puede trazar `@vercel/nft`, así que el archivo se declara
 * a mano en `vercel.json`:
 *
 *   "functions": { "api/mcp.mjs": { "includeFiles": "api/_generated/**" } }
 *
 * (La alternativa —`import ... with { type: 'json' }`— sí se trazaría sola,
 * pero deja el índice entero en el grafo de módulos ESM y no permite el
 * fallback de rutas de abajo, que es lo que hace testeable esto en local.)
 *
 * ─────────────────────────────────────────────────────────────────────────
 * EL EJE ES UN OBJETO DEL CONTRATO, NO UN DIALECTO
 *
 * Los tres repos de origen tenían el mismo motor con tres ejes distintos:
 * `version` (oba), ninguno (odumbo, `versionado: false`) y `project`
 * (adhoc). Acá el eje entra por el índice como el MISMO objeto que declara
 * `docs.config.json` (`{ tipo, default?, valores[] }`, ver
 * `schema/docs.config.schema.json` y `docs/unificacion/diseno-eje.md`), y el
 * artículo lo materializa en UN campo genérico: `articulos[].eje`
 * (`null` = fuera del eje). La palabra del dominio —`version` / `project`—
 * vive sólo en la superficie que ve el LLM: el nombre del parámetro de las
 * tools, el campo del hit y los textos. Ver `VOCABULARIO`.
 *
 * LA REGLA DEL LECTOR (contrato-indice.md §4): el índice declara
 * `schemaVersion` y este módulo TIRA si falta o si el emisor es más nuevo de
 * lo que sabe leer. No hay degradación blanda: un índice equivocado que
 * responde mal es peor que uno que no responde.
 */

import { readFileSync } from 'node:fs';
import { join } from 'node:path';
import { fileURLToPath } from 'node:url';

import MiniSearch from 'minisearch';

/** Versión del contrato índice ↔ motor que este módulo sabe leer. */
export const INDICE_SOPORTADO = 1;

/**
 * LOS DOS LLAMADORES DEL MOTOR, Y POR QUÉ NO COMPARTEN CONFIGURACIÓN.
 *
 * Este motor lo consulta un agente por MCP y —desde la task 75112— también una
 * persona en el buscador del sitio. Miden distinto, y está medido: lo que a uno
 * le sirve al otro le estorba.
 *
 *   - RESCATE DE TIPEO. El fuzzy estuvo apagado con este razonamiento (spec,
 *     review 19/08): "quien escribe la query es un LLM, que reformula limpio y
 *     reintenta gratis; sobre español el fuzzy mete falsos positivos. Queda
 *     tras este flag hasta que un caso real lo pida". Las dos mitades son
 *     ciertas. Una persona en el navbar NO reformula: escribe una vez, mira, y
 *     si no hay nada se va. Medido contra el índice publicado de oba-docs:
 *     `persepciones` devolvía CERO, y el buscador viejo del sitio la resolvía.
 *     Prendido globalmente rompe tres casos del golden set (`costo de merc`
 *     3º→4º, `facturación electrónica` 4º→6º con y sin tilde); como ÚLTIMO
 *     RECURSO no toca ninguno y rescata 7 de 7 tipeos probados, y los
 *     disparates (`asdfgh`) siguen dando cero.
 *   - RELLENO OR. Cuando el AND da cero, el motor afloja a OR y devuelve lo que
 *     matchea ALGUNOS términos, con una `nota` que pide verificar pertinencia.
 *     Para el agente es correcto y ahorra un viaje (fix #12). Para una persona
 *     es lo contrario: medido, `impresora fiscal epson` devuelve 124 resultados
 *     y `l10n_ar_afipws timeout` 391, y ninguna de las dos existe en el corpus.
 *     391 resultados sobre algo que no documentamos es PEOR que "no
 *     encontramos nada", porque los primeros parecen respuestas.
 *   - NOTA DE POBREZA. Cuando queda UN solo candidato el motor lo dice en una
 *     `nota`. Es prosa escrita para un lector que la lee y puede actuar: le
 *     pide verificar pertinencia y no concluir "no está documentado" sin antes
 *     aflojar el filtro. Una persona no lee eso —y no debería: para ella la
 *     honestidad es la interfaz, no un párrafo—, así que su perfil la apaga.
 *     Es la razón por la que esto es una capacidad del perfil y no un `if`
 *     sobre el modo: el caso es alcanzable por los dos llamadores.
 *
 * SE DECLARA, NO SE DETECTA. El perfil es un parámetro de `buscar()`, no algo
 * que el motor adivine por un header o un user-agent: eso es frágil en los dos
 * sentidos y deja el comportamiento colgado de algo que el llamador no declara.
 *
 * Y LO ATA QUIEN INYECTA EL MOTOR, no quien escribe la query. `crearMcp()`
 * recibe el módulo del índice inyectado, así que el handler del agente nunca
 * expone el perfil en el schema de su tool — si fuera un campo más de la tool,
 * el LLM podría ponerlo mal. El sitio pasa `PERFIL.persona` desde su propio
 * llamador.
 *
 * El default es `agente`: así todo lo que ya existía sigue igual sin tocar una
 * línea, y el que se tiene que declarar es el caso nuevo.
 */
export const PERFIL = {
  agente: { rescateDeTipeo: false, rellenoOr: true, notaDePobreza: true },
  persona: { rescateDeTipeo: true, rellenoOr: false, notaDePobreza: false },
};

/** Tope de hits por página de `buscar()`. */
export const PAGINA_BUSCAR = 20;

/** Tope de artículos por página de `mapa({ seccion })`. */
export const PAGINA_MAPA = 50;

/** Umbral de paginado del body. El límite real de Vercel es 4,5 MB por
 *  respuesta; con 200K caracteres por página quedamos un orden de magnitud
 *  abajo aun contando el escape de JSON. En la práctica no debería activarse. */
export const PAGINA_LEER = 200_000;

// ───────────────────────────────────────────────────────────── el eje

/** Los tres tipos de eje del contrato. */
export const TIPOS_DE_EJE = ['version', 'project', 'none'];

/**
 * Traduce el objeto `eje` del contrato al descriptor que consume el resto del
 * módulo. Es la única función que sabe qué implica cada `tipo`.
 *
 * Entra: `build.eje = { tipo, default?, valores? }` — el mismo objeto de
 * `docs.config.json`, emitido por el build (contrato-indice.md §3.3).
 *
 * Campos del descriptor:
 *   · `tipo`     `version` | `project` | `none`.
 *   · `hay`      hay eje (todo menos `none`).
 *   · `campo`    propiedad del artículo que lo materializa: `eje`, o `null`
 *                si el corpus no tiene eje. Genérica a propósito: es UN campo
 *                y no `version`/`project`, así que `pasaFiltros`, `hitDe`,
 *                `leer()` y los hints son UN código y no tres.
 *   · `param`    la palabra del DOMINIO: el nombre del parámetro en las tools
 *                y del campo en las respuestas (`version` / `project`).
 *   · `default`  el valor que gana cuando nadie lo pide. Su PRESENCIA es la
 *                política de desambiguación de `leer()` (diseno-eje.md §3).
 *   · `comodin`  un artículo con el campo en `null` aplica a TODOS los
 *                valores del eje (el caso cross-version de oba). Con eje
 *                `project` es `false`: un doc sin project no es "de todos".
 *   · `orden`    cómo se ordena la lista de valores (desc para versiones,
 *                alfabético para projects: entre projects no hay "el nuevo").
 *   · `sinValor` qué hace `leer()` sin valor del eje pedido: `default` (hay
 *                a quién elegir, y se ANUNCIA) o `ambiguo` (no lo hay: se
 *                devuelve la ambigüedad en vez de elegir por orden de
 *                inserción del índice).
 *   · `avisoMezcla`  `buscar()` sin filtro que trae varios valores del eje
 *                avisa que están mezclados, para que el agente atribuya antes
 *                de redactar. Encendido para los dos ejes (diseno-eje.md §3:
 *                mezclar la 18 con la 19 sin atribuir es tan caro como
 *                mezclar `oba` con `odumbo`).
 *   · `hintAgrupaPorEje` los hints agrupan por `eje/seccion` en vez de por
 *                `seccion` sola (tiene sentido cuando el eje es la unidad de
 *                atribución: projects).
 *   · `filtrosDominio` / `camposDominio` facetas de dominio del corpus, que
 *                salen de `build.metadata` (`modules` en OBA/Odumbo, `type`
 *                en adhoc-docs, `paises` en OBA). No son ejes: un artículo
 *                pertenece a UN valor del eje y a N valores de una faceta.
 *   · `dominiosComodin` las facetas cuya AUSENCIA significa "todos" en vez de
 *                "ninguno" (ver `pasaFiltros`). `paises` es la única hoy;
 *                `modules` NO: un artículo que no declara módulos no es "de
 *                todos los módulos".
 *
 * `paises` NO es un eje y por eso no entra en `TIPOS_DE_EJE`: no multiplica el
 * build ni bifurca URLs (frontmatter.md § "Países: una faceta, no un eje"). Es
 * un campo que viaja al índice y se filtra duro, como `modules`.
 */
export function politicaDeEje(build) {
  const declarado = build?.eje;
  if (!declarado || typeof declarado !== 'object' || Array.isArray(declarado)) {
    throw new Error(
      'el índice no declara `build.eje` como objeto { tipo, default?, valores? }: ' +
        'lo emitió un build pre-unificación (el `versionado`/`projects`/`latest` de los tres ' +
        'dialectos ya no se lee). Corré el build del paquete actual.',
    );
  }
  if (!TIPOS_DE_EJE.includes(declarado.tipo)) {
    throw new Error(
      `\`build.eje.tipo\` = ${JSON.stringify(declarado.tipo)} no es un tipo de eje: ` +
        `esperaba ${TIPOS_DE_EJE.join(' | ')}.`,
    );
  }

  const tipo = declarado.tipo;
  const hay = tipo !== 'none';
  const metadata = build?.metadata ?? {};
  const conModules = metadata.modules === true;
  const conTypes = Array.isArray(metadata.types) && metadata.types.length > 0;
  // El vocabulario de países lo declara el corpus (`metadata.paises`). Lista
  // vacía o ausente = el corpus no tiene la faceta y el MCP no ofrece el
  // filtro: una tool que ofrece un filtro sin contenido detrás miente.
  const conPaises = Array.isArray(metadata.paises) && metadata.paises.length > 0;
  /**
   * Los doc sets: varias documentaciones conviviendo en el mismo corpus (ej.
   * «Manual de usuario» y «Novedades de versión»).
   *
   * Lo declara el ÍNDICE (`build.metadata.docSets`), no el config del repo, por
   * lo mismo que `paises`: una declaración no se entera de que el contenido se
   * movió. Lista vacía o ausente = el corpus no tiene doc sets y la tool no
   * ofrece el filtro.
   *
   * ES ORTOGONAL AL EJE, no un segundo eje. `diseno-eje.md` §7 anticipó que el
   * día que hiciera falta `project × version` la salida sería un
   * `schemaVersion: 2` con `ejes` plural — esto NO es ese caso y no conviene
   * leerlo así: un doc set no discrimina el mismo contenido por otra dimensión,
   * agrupa secciones distintas. Cada doc set se sirve en TODOS los valores del
   * eje, y el eje sigue siendo uno solo por corpus.
   *
   * SIN COMODÍN, a diferencia de `paises`: todo artículo pertenece a exactamente
   * un doc set, así que la ausencia del campo no significa «todos» — significa
   * que el build no lo emitió. La asimetría es la trampa de este módulo y acá se
   * resuelve del lado de `modules`, no del de `paises`.
   */
  const conDocSets = Array.isArray(metadata.docSets) && metadata.docSets.length > 0;

  return {
    tipo,
    hay,
    campo: hay ? 'eje' : null,
    param: hay ? tipo : null,
    default: hay ? (declarado.default ?? null) : null,
    valoresDeclarados: Array.isArray(declarado.valores) ? declarado.valores : [],
    comodin: tipo === 'version',
    orden: tipo === 'version' ? 'desc' : 'asc',
    sinValor: hay && declarado.default != null ? 'default' : 'ambiguo',
    avisoMezcla: hay,
    hintAgrupaPorEje: tipo === 'project',
    filtrosDominio: [
      ...(conModules ? ['modules'] : []),
      ...(conPaises ? ['paises'] : []),
      ...(conDocSets ? ['docSet'] : []),
    ],
    camposDominio: [
      ...(conModules ? ['modules'] : []),
      ...(conPaises ? ['paises'] : []),
      ...(conTypes ? ['type'] : []),
      ...(conDocSets ? ['docSet'] : []),
    ],
    dominiosComodin: new Set(conPaises ? ['paises'] : []),
    paisesDeclarados: conPaises ? metadata.paises.map(String) : [],
    // `avisoMezclaDocSet` es hermano de `avisoMezcla` y existe por el mismo
    // motivo: mezclar en una respuesta el manual con las novedades de versión
    // sin atribuir es el error caro de este corpus — el agente cita una nota de
    // cambio como si fuera el procedimiento vigente.
    docSetsDeclarados: conDocSets
      ? metadata.docSets.map((d) => (typeof d === 'string' ? { id: d, label: d } : { id: String(d.id), label: String(d.label ?? d.id) }))
      : [],
    avisoMezclaDocSet: conDocSets,
  };
}

/**
 * Los nombres y los textos que hablan el idioma del dominio. El índice y el
 * config hablan genérico (`eje`); las tools y las respuestas que lee el LLM
 * hablan `version` o `project` (diseno-eje.md §4): el nombre del parámetro es
 * prompt, y "otra versión de lo mismo" no es lo mismo que "otro documento que
 * comparte nombre de archivo".
 *
 * Lo que SÍ se unificó de nombre (diseno-eje.md §3): `otrosDelEje` (era
 * `otrasVersiones` / `mismoSlugEnOtrosProjects`) y los `motivo` de los
 * soft-fail (`slug-fuera-del-valor-pedido`, `ambiguo-en-eje`), que son
 * machine-readable y no tienen por qué diferir por corpus.
 */
const VOCABULARIO = {
  version: {
    pedido: 'versionPedida',
    disponibles: 'versionesDisponibles',
    hintAflojar: (valores, cuantos, deDonde) =>
      `Sacando el filtro \`version\` (${valores.join(', ')}) hay ${cuantos} resultado(s), ` +
      `de: ${deDonde.join(', ')}. ` +
      'Ojo: eso devuelve otras versiones de Odoo — decilo explícitamente si las citás.',
    noCoincide: (slug, valor) =>
      `El slug "${slug}" existe, pero no para la versión ${valor}. ` +
      'La versión pedida manda: no te devuelvo otra sin que la pidas.',
    ambiguo: (slug, cuantos, valores) =>
      `El slug "${slug}" existe en ${cuantos} versiones (${valores.join(', ')}) y no me dijiste ` +
      'cuál. No elijo por mi cuenta: el corpus no declara una versión por default ' +
      '(`eje.default` en docs.config.json), y devolverte la equivocada te haría citar ' +
      'documentación de otra versión de Odoo. Repetí el `leer()` con la `version` que corresponda.',
    elegido: (valor) =>
      `No pediste versión: te devuelvo la ${valor}, que es la declarada por default en el corpus. ` +
      'Las otras están en `otrosDelEje` — si el usuario preguntó por otra, repetí el `leer()`.',
    mezcla: (valores) =>
      `Los resultados vienen de ${valores.length} versiones distintas (${valores.join(', ')}). ` +
      'Cada hit trae su `version`: citá siempre de cuál es, o volvé a buscar con el filtro ' +
      '`version` puesto si ya sabés sobre cuál te preguntaron.',
  },
  project: {
    pedido: 'projectPedido',
    disponibles: 'projectsDisponibles',
    // El aviso que con eje versión es "ojo, eso devuelve otras versiones de
    // Odoo". Acá el riesgo no es citar una versión vieja: es citar la
    // documentación de OTRO project como si fuera del que preguntaron.
    // `oba` y `odumbo` describen temas parecidos con criterios propios, y un
    // párrafo de uno leído como del otro es una respuesta creíble y
    // equivocada. Por eso el hint NOMBRA los projects de donde salen los
    // resultados: el agente tiene que poder atribuir sin volver a buscar.
    hintAflojar: (valores, cuantos, deDonde) =>
      `Sacando el filtro \`project\` (${valores.join(', ')}) hay ${cuantos} ` +
      `resultado(s), pero son de otro(s) project(s): ${deDonde.join(', ')}. ` +
      'La documentación de projects distintos habla de temas parecidos con criterios ' +
      'propios, así que NO sirve como respuesta sobre ' +
      `${valores.join(', ')}: si la citás, decí de qué project es y aclará que ` +
      'no es la del que te preguntaron.',
    noCoincide: (slug, valor) =>
      `El slug "${slug}" existe, pero no en el project "${valor}". ` +
      'El project pedido manda: no te devuelvo el artículo de otro sin que lo pidas, ' +
      'porque la documentación de dos projects distintos no es intercambiable.',
    ambiguo: (slug, cuantos, valores) =>
      `El slug "${slug}" existe en ${cuantos} projects (${valores.join(', ')}) y no me dijiste ` +
      'cuál. No elijo por mi cuenta: entre projects no hay uno "por default", y devolverte el ' +
      'equivocado te haría citar la documentación de otro producto. Repetí el `leer()` ' +
      'con el `project` que corresponda.',
    elegido: (valor) =>
      `No pediste project: te devuelvo "${valor}", que es el declarado por default en el corpus.`,
    mezcla: (valores) =>
      `Los resultados vienen de ${valores.length} projects distintos (${valores.join(', ')}). ` +
      'Cada hit trae su `project`: citá siempre de cuál es, o volvé a buscar con el filtro ' +
      '`project` puesto si ya sabés sobre cuál te preguntaron.',
  },
};

/**
 * El eje que expone `mapa()`: el MISMO objeto del contrato, más los valores
 * presentes en el índice y los declarados por la config. Un valor declarado
 * que no aparece no tiene docu o su fetch falló — y eso se ve acá (fix de
 * observabilidad de adhoc-docs).
 *
 * Con `tipo: 'none'` no viaja nada más: un `valores: []` con un `default: null`
 * al lado invita al agente a preguntarse cuál falta (criterio de odumbo-docs).
 */
function ejeEnMapa(idx) {
  const p = idx.eje;
  if (!p.hay) return { eje: { tipo: 'none' } };
  const eje = { tipo: p.tipo, param: p.param, valores: idx.valoresDeEje };
  if (p.default != null) eje.default = p.default;
  if (idx.ejeDeclarado.length) eje.declarados = idx.ejeDeclarado;
  return { eje };
}

// ─────────────────────────────────────────────────── normalización y términos

/**
 * Normalización compartida entre la construcción del índice y la query.
 * MiniSearch no trae nada de español de fábrica: esto, las STOPWORDS de abajo y
 * el reductor de plurales es todo lo que hay, y build y runtime tienen que usar
 * exactamente la misma función (stemming no hay, y a propósito: ver
 * `reducirPlural`).
 *
 * Minúsculas + sin tildes (NFD y fuera los diacríticos), para que
 * "posicion fiscal" matchee "posición fiscal".
 */
export function normalizarTermino(termino) {
  return termino
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/^[^\p{L}\p{N}]+|[^\p{L}\p{N}]+$/gu, '');
}

/**
 * Stopwords del español, corta y conservadora. (Fix #11 de oba-docs.)
 *
 * POR QUÉ: la query no siempre la escribe alguien que tipea keywords — la
 * escribe un LLM que a veces pega la frase literal del cliente ("quiero
 * suscribirme al enterprise"). `buscar()` combina los términos con AND, así
 * que cada palabra de relleno es un filtro más que el artículo tiene que
 * cumplir; encima, con `prefix: true`, "de"/"la" matchean medio corpus sin
 * aportar señal. Sacar el relleno es lo que hace que la frase conversacional
 * recupere la página correcta.
 *
 * Va escrita SIN tildes a propósito: se compara contra el término YA
 * normalizado por `normalizarTermino`.
 *
 * CRITERIO DE INCLUSIÓN: solo palabras de función (artículos, preposiciones,
 * pronombres, demostrativos, interrogativos) y los verbos de encuadre de una
 * frase conversacional ("quiero", "necesito", "puedo", "hago"). Ante la duda
 * NO entra: una palabra que también es término del dominio pierde precisión
 * si la borramos. Quedaron AFUERA a propósito "no" y "sin" (aparecen con
 * sentido en las `keywords:` del corpus — "no sale el cae", "probar sin
 * romper") y "sobre".
 *
 * La MISMA función se aplica al indexar y al buscar: va en el `processTerm`
 * del constructor de MiniSearch, que es también el que usa `search()` salvo
 * que se pise en `searchOptions` (no lo pisamos).
 */
export const STOPWORDS = new Set([
  // artículos y determinantes
  'el', 'la', 'los', 'las', 'lo', 'un', 'una', 'unos', 'unas', 'al', 'del',
  // preposiciones y conjunciones
  'a', 'con', 'de', 'desde', 'en', 'entre', 'hasta', 'para', 'por', 'segun',
  'y', 'o', 'que',
  // pronombres y posesivos
  'me', 'mi', 'mis', 'te', 'tu', 'tus', 'se', 'su', 'sus', 'le', 'les', 'nos',
  // demostrativos
  'este', 'esta', 'esto', 'estos', 'estas', 'ese', 'esa', 'eso',
  // interrogativos y relativos
  'como', 'cuando', 'donde', 'cual', 'cuales', 'quien',
  // verbos de encuadre de la frase conversacional
  'quiero', 'queria', 'quisiera', 'necesito', 'puedo', 'podria', 'hago', 'hace',
  'hacer', 'tengo', 'hay', 'saber', 'es', 'son', 'ser',
]);

/**
 * Sufijos de plural en "-es" que SÍ se recortan: las terminaciones donde el
 * singular castellano es inequívocamente consonante (-ón, -al, -or, -ad, -én,
 * -el/-il/-ol/-ul). El resto de los "-es" NO entra a propósito: "-tes", "-mes",
 * "-bres", "-des", "-lles", "-bles" son plurales de palabras que terminan en
 * "-e" ("cliente", "informe", "nombre", "tarde", "detalle", "variable"), y ahí
 * el recorte correcto es el de la "s" sola. La `l` doble queda afuera de
 * `[eiou]les` por eso mismo: "controles" es de "control", pero "detalles" es de
 * "detalle".
 */
const PLURAL_ES = /(?:ones|ales|ores|ades|enes|[eiou]les)$/;

/**
 * Reductor de plurales castellanos. NO es un stemmer y no queremos que lo sea:
 * un stemmer español completo también recorta derivación y conjugación, y ahí
 * empieza a fabricar fantasmas ("deseada" → "des" → "de"). Esto cubre UNA cosa,
 * la que hoy es una regresión frente al buscador del sitio: que `clientes` no
 * traiga «Facturas de cliente» y `cobros` no traiga «Registrar un cobro».
 *
 * POR QUÉ HACE FALTA si ya hay `prefix: true`. El prefijo cubre UN sentido: la
 * query `cliente` matchea el término indexado `clientes` porque el término
 * EMPIEZA con la query. Al revés no: `clientes` no matchea `cliente`, que es
 * más corto. Llevando las dos formas a la misma clave, los dos sentidos quedan
 * cubiertos con el mismo índice.
 *
 * El castellano forma el plural de dos maneras y el reductor tiene esas dos:
 *
 *   1. "+s" tras vocal átona (factura→facturas, cobro→cobros, cliente→clientes).
 *      Sacar la "s" final es MONÓTONO respecto del prefijo: recorta un solo
 *      carácter, así que nunca puede romper un match que el prefijo ya daba
 *      (medido sobre el vocabulario publicado: 0 pares perdidos).
 *   2. "+es" tras consonante (percepción→percepciones, total→totales). Acá hay
 *      que sacar DOS caracteres, y eso sí puede romper el prefijo cuando el
 *      singular termina en "-e" — de ahí la lista cerrada de `PLURAL_ES`.
 *
 * Los tres guards:
 *   · menos de 5 caracteres no se toca: recortar destruye el término ("mes",
 *     "pais", "tres", "res") y, peor, acorta la QUERY, que con `prefix: true`
 *     pasa a pescar medio corpus ("mes" → "me" → "mercado", "medios", "metodo").
 *   · "-is"/"-us" no son plurales: "analisis", "crisis", "status", "virus".
 *   · "-ss" tampoco, y es el inglés que hay en el corpus: "access", "discuss".
 *
 * Deliberadamente AFUERA: "-ces" → "-z" ("lápices"→"lápiz", "veces"→"vez").
 * Se midió y no cierra: el castellano tiene demasiados singulares en "-ce" y la
 * regla rompe `enlace`→«enlaces», `índice`→«índices», `balance`→«balances»,
 * `avance`→«avances» y `invoice`→«invoices», que hoy funcionan por prefijo. Lo
 * que compra (nadie busca "veces") no paga lo que rompe.
 *
 * EL COSTO QUE ESTO TIENE, sin taparlo: la consulta `sobres` pasa a reducirse a
 * `sobre`, que en este corpus NO es relleno —aparece en 8 títulos («Acciones
 * sobre contactos», «Consideraciones especiales sobre IVA») y en 9 `keywords:`
 * («IVA sobre intereses», «sobre de documentos»)—, así que quien busque el
 * sobre físico ahora también pesca esas páginas. Idem `antes` → `ante`.
 *
 * El arreglo limpio sería mandar `sobre` a STOPWORDS, y NO se hace: quedó
 * afuera de esa lista a propósito y hay un test que lo fija (`buscar.test.mjs`,
 * "las palabras del dominio que también parecen relleno NO se descartan"),
 * justamente porque en este corpus tiene sentido. Entre perder una consulta
 * marginal y romper la de un término que el corpus usa de verdad, se elige
 * perder la consulta. Si `sobres` apareciera en las consultas reales, la salida
 * es una excepción acá, no cambiar las STOPWORDS.
 */
export function reducirPlural(termino) {
  if (termino.length < 5) return termino;
  if (!termino.endsWith('s')) return termino;
  if (/(?:is|us|ss)$/.test(termino)) return termino;
  if (termino.length >= 7 && PLURAL_ES.test(termino)) return termino.slice(0, -2);
  return termino.slice(0, -1);
}

/**
 * El `processTerm` que comparten índice y query: normaliza, descarta relleno y
 * unifica el plural con el singular.
 * Devolver `null` le dice a MiniSearch que el término no existe — tanto al
 * indexar como al buscar.
 *
 * El orden importa. `reducirPlural` va DESPUÉS de `normalizarTermino` (que ya
 * sacó las tildes) porque es lo que hace que "percepciones" y "percepción"
 * lleguen a la misma clave: sin sacar la tilde primero habría que escribir las
 * reglas dos veces. Y va DESPUÉS de las STOPWORDS, que están escritas en su
 * forma de superficie: reducir antes convertiría "cuales" en "cuale", que ya no
 * está en la lista y entraría como término con señal.
 */
export function procesarTermino(termino) {
  const limpio = normalizarTermino(termino);
  if (!limpio.length) return null;
  if (STOPWORDS.has(limpio)) return null;
  return reducirPlural(limpio);
}

/** Los términos con señal de una query. Para diagnóstico y para los hints. */
export function terminosDe(q) {
  return String(q || '')
    .split(/\s+/)
    .map(procesarTermino)
    .filter(Boolean);
}

// ──────────────────────────────────────────────────────── carga del índice

const CAMPOS_INDEXADOS = ['title', 'keywords', 'headingsText', 'description', 'body'];

// Boost por campo: título y keywords arriba (las keywords son el vocabulario
// del usuario, nutrido de las queries reales — spec §La búsqueda).
const BOOST = {
  title: 6,
  keywords: 5,
  headingsText: 3,
  description: 2,
  body: 1,
};

const CAMPOS_GUARDADOS_BASE = ['slug', 'title', 'description', 'url', 'headings', 'seccion'];

/**
 * Las opciones con las que se construye el índice, en UNA llamada.
 *
 * POR QUÉ SE EXPORTA. El preprocesador de un corpus necesita construir el mismo
 * índice que este motor lee, y hasta acá los campos, los boosts y los campos
 * guardados eran constantes privadas de este archivo: para usarlas había que
 * copiarlas. Una configuración copiada diverge igual que el código copiado, y
 * este paquete ya pagó ese precio una vez — es lo que cuenta el docstring de
 * `lib/busqueda.cjs` sobre la config del tema copiada en tres repos.
 *
 * POR QUÉ DEVUELVE EL OBJETO ENTERO Y NO LAS CONSTANTES SUELTAS. Exportar
 * `CAMPOS_INDEXADOS`, `BOOST` y `CAMPOS_GUARDADOS_BASE` para que el llamador los
 * ensamble deja el ENSAMBLADO como un segundo lugar donde divergir: alcanza con
 * olvidarse el `processTerm` para que el índice emitido y el que este motor
 * construye tokenicen distinto, y eso no falla — devuelve otros resultados.
 * Acá el llamador no ensambla nada: `new MiniSearch(opcionesDelIndice(build))`.
 *
 * NO ES el `OPCIONES_DE_INDICE` que se borró en v0.8.0. Ese existía para que un
 * bin reconstruyera índices de LUNR con el pipeline del tema, y se fue porque
 * tenía cero consumidores. Esto es del motor propio y tiene uno.
 *
 * `build` es el bloque `build` del índice: de ahí sale la política del eje, que
 * decide qué campos se guardan además de los de base.
 */
export function opcionesDelIndice(build) {
  const politica = politicaDeEje(build);
  return {
    idField: 'id',
    fields: [...CAMPOS_INDEXADOS],
    storeFields: [
      ...CAMPOS_GUARDADOS_BASE,
      ...(politica.campo ? [politica.campo] : []),
      ...politica.camposDominio,
    ],
    // La misma función al indexar y al buscar (MiniSearch reusa este
    // processTerm en `search()` mientras no se pise en searchOptions).
    processTerm: procesarTermino,
    searchOptions: {
      boost: { ...BOOST },
      prefix: true,
      // El fuzzy NO se fija acá: lo decide cada llamada según su perfil (ver
      // `PERFIL`). MiniSearch FUSIONA las opciones de `search()` sobre estas
      // —verificado: pasar sólo `fuzzy` no pierde `boost` ni `prefix`—, así que
      // el default de acá vale para quien no lo pise.
      fuzzy: false,
    },
  };
}

function rutasCandidatas() {
  const rutas = [];
  if (process.env.DOCS_INDICE_PATH) rutas.push(process.env.DOCS_INDICE_PATH);
  // El consumidor: el índice vive en `api/_generated/` del repo que buildea.
  rutas.push(join(process.cwd(), 'api', '_generated', 'index.json'));
  // Layout viejo (el módulo dentro del repo, en lib/mcp/). Se conserva para
  // que el paquete siga funcionando vendorizado.
  rutas.push(fileURLToPath(new URL('../../api/_generated/index.json', import.meta.url)));
  return rutas;
}

function leerIndiceCrudo() {
  const errores = [];
  for (const ruta of rutasCandidatas()) {
    try {
      return JSON.parse(readFileSync(ruta, 'utf8'));
    } catch (error) {
      errores.push(`${ruta}: ${error.code || error.message}`);
    }
  }
  throw new Error(
    `No se pudo leer el índice del agente. Rutas probadas — ${errores.join(' | ')}`,
  );
}

/**
 * La regla del lector (contrato-indice.md §4). Tirar y no degradar: los
 * lectores viejos tenían fallbacks blandos que convertían un índice
 * equivocado en un índice que responde mal.
 */
function verificarContrato(crudo) {
  if (!Number.isInteger(crudo?.schemaVersion)) {
    throw new Error(
      'índice sin `schemaVersion`: lo emitió un build pre-unificación. ' +
        'Corré el build del paquete actual.',
    );
  }
  if (crudo.schemaVersion > INDICE_SOPORTADO) {
    throw new Error(
      `el índice declara schemaVersion ${crudo.schemaVersion} y esta plataforma lee hasta ` +
        `${INDICE_SOPORTADO}: actualizá @ingadhoc/docs-platform en este repo.`,
    );
  }
  // Los dos artefactos del build difieren SÓLO en la presencia de `body` y son
  // indistinguibles por contenido. Si la función MCP termina leyendo la copia
  // estática, `leer()` devolvería `body: ''` con `encontrado: true`: artículos
  // vacíos con cara de artículo. Con el flag, falla fuerte y con nombre.
  if (crudo.build?.conCuerpo === false) {
    throw new Error(
      'este índice viene sin `body` (`build.conCuerpo: false`, es la copia estática de ' +
        '`site/static/agente/`): la función MCP necesita `api/_generated/index.json`.',
    );
  }
}

/**
 * Los valores del eje declarados por la config vía el build. Con el contrato
 * versionado la forma está fijada (`{id, label?}`), así que el lector NO
 * adivina: un valor sin `id` es un índice mal emitido y se dice.
 * Sólo los valores ACTIVOS viajan al índice: los apagados no son contenido,
 * son registro (contrato-indice.md §3.3).
 */
function ejeDeclarado(politica) {
  return politica.valoresDeclarados.map((valor, i) => {
    if (!valor || typeof valor !== 'object' || !valor.id) {
      throw new Error(`\`build.eje.valores[${i}]\` no tiene \`id\`: el índice está mal emitido.`);
    }
    return { id: String(valor.id), label: String(valor.label ?? valor.id) };
  });
}

function construir() {
  const crudo = leerIndiceCrudo();
  verificarContrato(crudo);

  const articulos = Array.isArray(crudo.articulos) ? crudo.articulos : [];
  const politica = politicaDeEje(crudo.build);

  const mini = new MiniSearch(opcionesDelIndice(crudo.build));

  // El `id` lo EMITE el build (`${eje ?? '*'}::${slug}`, diseno-eje.md §2). El
  // lector no lo compone: que el identificador se invente en el lector es como
  // los tres repos emitían de hecho ids `"null::relacion/…"`.
  for (const articulo of articulos) {
    if (!articulo.id) {
      throw new Error(
        `el artículo "${articulo.slug}" viene sin \`id\`: lo emite el build ` +
          '(`${eje ?? "*"}::${slug}`) y el lector no lo compone. Corré el build del paquete actual.',
      );
    }
  }

  const vistosPorSlug = new Map();
  /**
   * Con doc sets, `leer({slug})` NO gana una segunda dimensión de ambigüedad —
   * y eso no es suerte, es un invariante que se verifica acá.
   *
   * Un doc set agrupa secciones, y la sección es el primer segmento del slug:
   * `manual/…` nunca puede ser también `novedades/…`. Por eso el `id` sigue
   * siendo `${eje}::${slug}` sin tocar el formato, y `leer()` sigue teniendo UNA
   * regla y no un `if` por doc set.
   *
   * QUÉ AGREGA ESTE CHEQUEO SOBRE LO QUE YA HAY. MiniSearch también rechaza dos
   * documentos con el mismo `id`, así que el caso no pasaba desapercibido — pero
   * su mensaje es `duplicate ID 19::manual/…`, que no dice ni que hay doc sets
   * ni dónde mirar. Acá se falla antes y con la causa: casi siempre es una
   * sección declarada en dos `docSets.secciones`.
   */
  for (const articulo of articulos) {
    const clave = `${articulo[politica.campo] ?? '*'}::${articulo.slug}`;
    const previo = vistosPorSlug.get(clave);
    if (previo !== undefined && previo !== (articulo.docSet ?? null)) {
      throw new Error(
        `el índice trae el slug ${JSON.stringify(articulo.slug)} en más de un doc set ` +
          `(${JSON.stringify(previo)}, ${JSON.stringify(articulo.docSet ?? null)}) para el mismo ` +
          'valor de eje. Un doc set agrupa secciones y la sección es el primer segmento del slug: ' +
          'dos doc sets no pueden compartir un slug. Revisá `docSets.secciones` en el contrato del ' +
          'corpus — si dos doc sets declaran la misma sección, el build las emite mezcladas.',
      );
    }
    vistosPorSlug.set(clave, articulo.docSet ?? null);
  }

  mini.addAll(
    articulos.map((articulo) => ({
      ...articulo,
      keywords: (articulo.keywords || []).join(' '),
      headingsText: (articulo.headings || []).map((h) => h.text).join(' '),
      body: articulo.body || '',
    })),
  );

  const porId = new Map(articulos.map((a) => [a.id, a]));
  const porSlug = new Map();
  for (const articulo of articulos) {
    if (!porSlug.has(articulo.slug)) porSlug.set(articulo.slug, []);
    porSlug.get(articulo.slug).push(articulo);
  }


  const declarados = ejeDeclarado(politica);
  const labels = new Map(declarados.map((p) => [p.id, p.label]));

  // Orden del eje: descendente para versiones ("la última primero"),
  // alfabético para projects — entre projects no hay "el más nuevo".
  const valores = politica.campo
    ? [...new Set(articulos.map((a) => a[politica.campo]))].filter(Boolean).sort()
    : [];
  if (politica.orden === 'desc') valores.reverse();

  return {
    build: crudo.build || {},
    schemaVersion: crudo.schemaVersion,
    eje: politica,
    buildId: crudo.build?.generatedAt || 'desconocido',
    // `audiencia` es la clave del contrato, en castellano y una sola (dos
    // claves para el mismo dato es la doble fuente que se pudre). El fallback
    // a la env var es para un índice viejo o a medio generar — no es un
    // segundo nombre del mismo campo.
    audiencia: crudo.build?.audiencia || process.env.DOCS_AUDIENCE || 'desconocida',
    mapa: crudo.mapa || [],
    articulos,
    valoresDeEje: valores,
    ejeDeclarado: declarados,
    labelDeEje: (id) => labels.get(id) || id,
    mini,
    porId,
    porSlug,
  };
}

let cache = null;
let errorDeCarga = null;

/** Singleton de scope de módulo: una carga y un índice por instancia. */
export function indice() {
  if (cache) return cache;
  if (errorDeCarga) throw errorDeCarga;
  try {
    cache = construir();
    return cache;
  } catch (error) {
    errorDeCarga = error;
    throw error;
  }
}

/** Solo para tests: fuerza recargar. */
export function _resetIndice() {
  cache = null;
  errorDeCarga = null;
}

/**
 * URL canónica absoluta. El índice trae rutas relativas al sitio (`/19/...`,
 * `/adhoc-way/...`); el origin sale de DOCS_URL — la MISMA variable que ya
 * consume Docusaurus para su `url` y el preprocesador para `siteUrl`,
 * distinta en cada proyecto. No inventamos una variable nueva a propósito:
 * dos vars con el mismo valor se desincronizan. Sin la variable, la URL queda
 * relativa — citable dentro del sitio pero no pegable en un ticket.
 */
function absoluta(url) {
  if (!url) return url;
  const origin = (process.env.DOCS_URL || '').replace(/\/+$/, '');
  return origin ? origin + url : url;
}

function comoLista(valor) {
  if (valor == null) return null;
  const lista = Array.isArray(valor) ? valor : [valor];
  const limpia = lista.filter((v) => v != null && String(v).length > 0).map(String);
  return limpia.length ? limpia : null;
}

// ──────────────────────────────────────────────────────────── filtros duros

/**
 * Los campos por los que se puede filtrar duro en este corpus, con el NOMBRE
 * que ve el consumidor (`version`/`project`, no `eje`).
 */
function camposDeFiltro(politica) {
  return [...(politica.param ? [politica.param] : []), 'seccion', ...politica.filtrosDominio];
}

/**
 * Filtros duros por metadata. NO son términos de la query: un artículo que
 * no cumple queda afuera, no rankeado más abajo. El del eje es el que
 * manda — preguntar por la 19 nunca devuelve la 18 sin decirlo, y preguntar
 * por `oba` nunca devuelve `odumbo`.
 */
function pasaFiltros(articulo, filtros, politica) {
  const { campo, param } = politica;
  if (campo && filtros[param]) {
    // Fix #12 de oba-docs, generalizado con `politica.comodin`: un artículo
    // FUERA DEL EJE llega con `eje: null` (no declara valor porque aplica a
    // todos). `String(null)` es "null", que no está en ninguna lista de
    // valores: sin este caso, filtrar por versión lo excluía siempre — y la
    // skill del agente consumidor le ordena filtrar SIEMPRE por versión, así
    // que TODO el contenido cross-version (`relacion/`) quedaba invisible en
    // la práctica. Con eje `project` el comodín está APAGADO: un doc sin
    // project no es "de todos los projects".
    const valor = articulo[campo];
    const comodin = politica.comodin && valor == null;
    if (!comodin && !filtros[param].includes(String(valor))) return false;
  }
  if (filtros.seccion && !filtros.seccion.includes(articulo.seccion)) return false;
  for (const dominio of politica.filtrosDominio) {
    if (!filtros[dominio]) continue;
    const propios = articulo[dominio];
    const lista = propios == null ? [] : Array.isArray(propios) ? propios : [propios];
    // El MISMO Fix #12, una faceta más abajo. Una faceta COMODÍN (`paises`)
    // significa "todos" cuando el artículo no declara nada: una página
    // universal tiene que devolverse bajo cualquier filtro de país, o filtrar
    // por UY borra todo el contenido que no es de ningún país en particular —
    // que es la mayoría del manual. El tag excluye; la ausencia nunca oculta.
    // `modules` conserva la semántica opuesta: sin módulos declarados, no
    // matchea ningún filtro de módulo.
    if (lista.length === 0 && politica.dominiosComodin?.has(dominio)) continue;
    if (!filtros[dominio].some((m) => lista.map(String).includes(String(m)))) return false;
  }
  return true;
}

function normalizarFiltros(args, politica) {
  const filtros = {};
  for (const campo of camposDeFiltro(politica)) filtros[campo] = comoLista(args[campo]);
  return filtros;
}

function hitDe(resultado, politica) {
  const hit = {
    slug: resultado.slug,
    title: resultado.title,
    description: resultado.description,
    url: absoluta(resultado.url),
    seccion: resultado.seccion,
    // Los headings viajan en el hit para que el agente pueda citar el
    // deep-link al topic sin tener que leer() el artículo entero.
    headings: resultado.headings || [],
    score: Math.round(resultado.score * 1000) / 1000,
  };
  // El valor del eje viaja con la palabra del dominio: `version: "19"`.
  if (politica.campo) hit[politica.param] = resultado[politica.campo];
  for (const campo of politica.camposDominio) {
    // `paises: null` en el hit NO es "no sé": es "aplica a todos los países".
    // Por eso no se normaliza a `[]` como `modules` — un array vacío se lee
    // como "ningún país" y es justo lo contrario.
    hit[campo] = resultado[campo] ?? (campo === 'modules' ? [] : null);
  }
  return hit;
}

// ──────────────────────────────────────────────── fragmento y ancla del hit

/** Tope del `fragmento` de un hit, contando los "…" de los bordes. */
export const LARGO_FRAGMENTO = 280;

/** Cuánto contexto va ANTES del término que ancla la ventana. El resto va
 *  después: una oración se entiende mejor por lo que sigue que por lo previo. */
const CONTEXTO_PREVIO = 60;

/** Cuánto se permite correr un borde de la ventana para caer en un espacio.
 *  Más allá, se corta por caracteres: un tramo largo sin espacios no puede
 *  llevarse la ventana lejos del término. */
const MARGEN_DE_PALABRA = 20;

/** Las palabras de un texto, con la MISMA partición que el tokenizer por
 *  default de MiniSearch (`[\n\r\p{Z}\p{P}]+`): si partieran distinto, el
 *  término que el índice matcheó podría no aparecer al recorrer el cuerpo. */
const PALABRA = /[^\n\r\p{Z}\p{P}]+/gu;

/**
 * Una línea de markdown, en el texto que lee una persona. No es un parser: es
 * lo mínimo para que el fragmento no llegue con ruido — links a su texto,
 * imágenes y tags de componentes (MDX/Mintlify) afuera, sin énfasis, sin
 * backticks, sin marcas de lista, cita ni tabla. Lo que sobreviva de más es
 * texto que igual se entiende.
 *
 * Las clases negadas excluyen también el delimitador que ABRE (`[^\[\]]`,
 * `[^()]`, `[^<>]`): así cada intento de match termina en el próximo `[`, `(`
 * o `<`, y una línea con miles sin cerrar es lineal y no cuadrática.
 */
function textoPlano(linea) {
  return linea
    .replace(/!\[[^\[\]]*\]\([^()]*\)/g, ' ')
    .replace(/\[([^\[\]]*)\]\([^()]*\)/g, '$1')
    .replace(/<\/?[A-Za-z][^<>]*>/g, ' ')
    .replace(/\*\*|__|`/g, '')
    .replace(/^\s*(?:>\s*)+/, '')
    .replace(/^\s*(?:[-*+]|\d+[.)])\s+/, '')
    .replace(/\|/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}

/** Texto de heading comparable: sin markdown, sin id explícito `{#…}`,
 *  normalizado como el motor (minúsculas, sin tildes). */
function claveDeHeading(texto) {
  return normalizarTermino(textoPlano(texto.trimEnd().replace(/\{#[^{}]*\}$/, '')));
}

/**
 * Parte el cuerpo en bloques de texto plano, uno por heading h2/h3, cada uno
 * con su ancla (`null` para lo que está antes del primero). Los h4 en adelante
 * no abren bloque: quedan bajo el h2/h3 que los contiene, que es lo que un link
 * con `#` puede señalar con sentido.
 *
 * EL ANCLA SALE DE `articulo.headings`, NO SE RECALCULA. El id lo emitió el
 * build con las reglas de slug del sitio (Mintlify y Docusaurus no slugifican
 * igual), y un id recalculado acá sería un link que parece bueno y no abre. Se
 * aparea por texto normalizado y, si el texto no coincide —un heading con
 * markdown que el build limpió distinto—, por orden de aparición.
 *
 * Las líneas de código dentro de un fence cuentan como texto (el término puede
 * estar sólo ahí) pero nunca abren bloque: un `## ` adentro de un fence es
 * código, no un heading.
 */
function bloquesDelCuerpo(articulo) {
  let bloques = bloquesPorArticulo.get(articulo);
  if (!bloques) {
    bloques = partirCuerpo(articulo);
    bloquesPorArticulo.set(articulo, bloques);
  }
  return bloques;
}

/** El índice no cambia en el proceso: los bloques de un artículo se calculan
 *  una vez. `WeakMap` para que un `_resetIndice()` los suelte con el índice. */
const bloquesPorArticulo = new WeakMap();

/** Solo para tests: los bloques memorizados de un artículo. */
export const _bloquesDelCuerpo = (articulo) => bloquesDelCuerpo(articulo);

function partirCuerpo(articulo) {
  const headings = (articulo.headings || []).filter((h) => h.level === 2 || h.level === 3);
  let siguiente = 0;
  const bloques = [{ ancla: null, titulo: '', lineas: [] }];
  let enFence = false;
  for (const linea of String(articulo.body || '').split(/\r?\n/)) {
    if (/^\s*(?:```|~~~)/.test(linea)) {
      enFence = !enFence;
      continue;
    }
    const heading = enFence ? null : /^(#{1,6})\s+(.*)$/.exec(linea);
    if (heading && (heading[1].length === 2 || heading[1].length === 3)) {
      const clave = claveDeHeading(heading[2]);
      let i = headings.findIndex((h, j) => j >= siguiente && claveDeHeading(h.text) === clave);
      if (i < 0 && siguiente < headings.length) i = siguiente;
      const h = i >= 0 ? headings[i] : null;
      if (i >= 0) siguiente = i + 1;
      bloques.push({ ancla: h ? { id: h.id, text: h.text } : null, titulo: textoPlano(heading[2]), lineas: [] });
      continue;
    }
    // El h1 repite el título del artículo: no es cuerpo que valga citar.
    if (heading && heading[1].length === 1) continue;
    const plano = textoPlano(heading ? heading[2] : linea);
    if (plano) bloques.at(-1).lineas.push(plano);
  }
  return bloques.map((b) => ({ ancla: b.ancla, titulo: b.titulo, texto: b.lineas.join(' ') }));
}

/** Las apariciones de los términos buscados en un texto: posición y término. */
function apariciones(texto, terminos) {
  const out = [];
  for (const m of texto.matchAll(PALABRA)) {
    const t = procesarTermino(m[0]);
    if (t && terminos.has(t)) out.push({ pos: m.index, largo: m[0].length, termino: t });
  }
  return out;
}

const esAlto = (c) => c >= 0xd800 && c <= 0xdbff;
const esBajo = (c) => c >= 0xdc00 && c <= 0xdfff;

/**
 * Los bordes `[ini, fin)` de la ventana de `LARGO_FRAGMENTO` alrededor del
 * término en `pos` (de `largo` caracteres), contando los "…" que llevará.
 *
 * El término queda SIEMPRE adentro: cada borde busca un espacio a no más de
 * `MARGEN_DE_PALABRA` y, si no lo hay, corta por caracteres. Ningún borde
 * parte un par surrogate. Es la misma ventana que puntúa `fragmentoDe` y la
 * que devuelve `ventana`: si fueran distintas, el puntaje premiaría términos
 * que el fragmento no muestra.
 */
function limites(texto, pos, largo) {
  const total = texto.length;
  if (total <= LARGO_FRAGMENTO) return { ini: 0, fin: total };
  const lugar = (ini) => LARGO_FRAGMENTO - (ini > 0 ? 1 : 0) - 1;

  let ini = Math.max(0, pos - CONTEXTO_PREVIO);
  if (ini > 0) {
    const espacio = texto.lastIndexOf(' ', ini);
    if (espacio >= 0 && ini - espacio <= MARGEN_DE_PALABRA) ini = espacio + 1;
  }
  // Un término que no entra después de `ini` corre la ventana hacia él; si es
  // más largo que la ventana entera, arranca en él.
  if (pos + largo > ini + lugar(ini)) ini = Math.min(pos, pos + largo - lugar(1));
  if (ini > 0 && ini < total && esBajo(texto.charCodeAt(ini))) ini += 1;

  let fin = ini + lugar(ini);
  if (fin >= total) return { ini, fin: total };
  const espacio = texto.lastIndexOf(' ', fin);
  if (espacio >= pos + largo && fin - espacio <= MARGEN_DE_PALABRA) fin = espacio;
  if (esAlto(texto.charCodeAt(fin - 1))) fin -= 1;
  return { ini, fin };
}

/** Recorta `texto` a la ventana de `limites`, con "…" en cada borde cortado. */
function ventana(texto, pos, largo) {
  const { ini, fin } = limites(texto, pos, largo);
  return (ini > 0 ? '…' : '') + texto.slice(ini, fin).trim() + (fin < texto.length ? '…' : '');
}

/**
 * `fragmento`, `ancla` y `urlAncla` de un hit: el pedazo del cuerpo donde
 * aparece la consulta y el heading bajo el que cae, para que el agente pueda
 * citar el lugar exacto —y decidir si el hit sirve— sin `leer()` el artículo.
 *
 * QUÉ TÉRMINOS BUSCA. No re-procesa la query: usa `resultado.match`, que es lo
 * que MiniSearch matcheó y en qué campo. Así la comparación es la del motor por
 * construcción —normalizado, sin tildes, plural reducido, y con el prefijo ya
 * resuelto al término del índice—, y "el match fue sólo en el título" no hay
 * que adivinarlo: si ningún término matcheó en `body`, no hay fragmento.
 *
 * QUÉ APARICIÓN ELIGE. La primera de la ventana que junta MÁS términos
 * distintos de la consulta; a igual cantidad, la que los nombra más veces; y
 * entre ventanas empatadas en las dos cosas, la primera. "La primera
 * aparición de algún término" a secas elige mal justo en los artículos largos
 * que más lo necesitan: en la FAQ, `dos empresas claude` caía en la pregunta
 * sobre servidores de terceros (que nombra a Claude al pasar) y no en la que
 * habla de dos empresas.
 *
 * EL COSTO. Recorre el cuerpo entero, así que se llama sólo para los hits de
 * la página devuelta (≤ `PAGINA_BUSCAR`), nunca para todos los candidatos.
 */
function fragmentoDe(resultado, articulo, conCuerpo) {
  const nada = { fragmento: null, ancla: null, urlAncla: null };
  if (!conCuerpo || !articulo?.body) return nada;
  const terminos = new Set(
    Object.entries(resultado.match || {})
      .filter(([, campos]) => campos.includes('body'))
      .map(([termino]) => termino),
  );
  if (!terminos.size) return nada;

  const bloques = bloquesDelCuerpo(articulo);
  let mejor = null;
  for (const bloque of bloques) {
    const vistas = apariciones(bloque.texto, terminos);
    // Dos punteros sobre `vistas` (ordenadas por posición y sin solaparse,
    // así que los finales también están ordenados): la ventana de cada
    // aparición es vistas[lo, hi). Los bordes casi siempre avanzan, así que el
    // recorrido es lineal y no O(k²) con un término muy repetido.
    const cuenta = new Map();
    const sumar = (t, d) => {
      const n = (cuenta.get(t) || 0) + d;
      if (n) cuenta.set(t, n);
      else cuenta.delete(t);
    };
    let lo = 0;
    let hi = 0;
    let objLo = 0;
    let objHi = 0;
    for (const a of vistas) {
      const { ini, fin } = limites(bloque.texto, a.pos, a.largo);
      const termina = (i) => vistas[i].pos + vistas[i].largo;
      while (objLo < vistas.length && vistas[objLo].pos < ini) objLo += 1;
      while (objLo > 0 && vistas[objLo - 1].pos >= ini) objLo -= 1;
      while (objHi < vistas.length && termina(objHi) <= fin) objHi += 1;
      while (objHi > 0 && termina(objHi - 1) > fin) objHi -= 1;
      while (hi < objHi) sumar(vistas[hi++].termino, 1);
      while (lo > objLo) sumar(vistas[--lo].termino, 1);
      while (lo < objLo) sumar(vistas[lo++].termino, -1);
      while (hi > objHi) sumar(vistas[--hi].termino, -1);
      const puntaje = [cuenta.size, hi - lo];
      if (!mejor || puntaje[0] > mejor.puntaje[0] || (puntaje[0] === mejor.puntaje[0] && puntaje[1] > mejor.puntaje[1])) {
        mejor = { bloque, a, puntaje };
      }
    }
  }
  // El término puede estar sólo en un heading (el cuerpo lo indexa entero): el
  // fragmento es entonces el arranque de ese bloque, y el ancla, ese heading.
  if (!mejor) {
    const bloque = bloques.find((b) => apariciones(b.titulo, terminos).length);
    if (!bloque) return nada;
    mejor = { bloque, a: { pos: 0, largo: 0 } };
  }
  const { bloque, a } = mejor;
  const fragmento = bloque.texto ? ventana(bloque.texto, a.pos, a.largo) : null;
  const ancla = bloque.ancla;
  return {
    fragmento,
    ancla,
    urlAncla: ancla ? `${absoluta(articulo.url)}#${ancla.id}` : null,
  };
}

// ─────────────────────────────────────────────────────────────────── hints

/**
 * Hints accionables para el caso de CERO resultados de verdad: ni el AND ni el
 * fallback OR devolvieron nada. Para un LLM esto vale más que cualquier
 * tolerancia de matching: le decimos qué filtro sacar y qué secciones del mapa
 * tocan sus términos.
 */
function hints(idx, q, filtros) {
  const politica = idx.eje;
  const sugerencias = [];

  // 0. ¿La query quedó vacía de términos con señal? (todo stopwords)
  if (terminosDe(q).length === 0) {
    sugerencias.push(
      'Tu query no tiene ningún término con contenido: son todas palabras vacías ' +
        '(artículos, preposiciones, "quiero", "cómo hago"…), que no se indexan. ' +
        'Dejá los sustantivos del tema (p. ej. "nota de crédito", "conciliación bancaria").',
    );
  }

  // 1. ¿Hay resultados si aflojamos cada filtro de a uno?
  for (const campo of camposDeFiltro(politica)) {
    if (!filtros[campo]) continue;
    const sinEse = { ...filtros, [campo]: null };
    const otros = idx.mini.search(q, {
      combineWith: 'AND',
      filter: (r) => pasaFiltros(r, sinEse, politica),
    });
    if (otros.length === 0) continue;

    if (campo === politica.param) {
      const deDonde = [...new Set(otros.map((r) => r[politica.campo]))].sort();
      sugerencias.push(VOCABULARIO[politica.tipo].hintAflojar(filtros[campo], otros.length, deDonde));
    } else {
      sugerencias.push(
        `Sacando el filtro \`${campo}\` (${filtros[campo].join(', ')}) hay ${otros.length} ` +
          'resultado(s). Probá de nuevo sin ese filtro.',
      );
    }
  }

  // 2. ¿Alguno de los términos sueltos pega en algún lado, ignorando los
  //    filtros duros? (Con los filtros puestos ya lo intentó el fallback OR de
  //    `buscar()`: si llegamos acá, ahí no había nada.)
  const porTermino = idx.mini.search(q, { combineWith: 'OR' });
  const grupos = new Map();
  for (const r of porTermino.slice(0, 40)) {
    const clave = politica.hintAgrupaPorEje
      ? `${r[politica.campo] || '—'}/${r.seccion || '—'}`
      : `${r.seccion || '—'}`;
    grupos.set(clave, (grupos.get(clave) || 0) + 1);
  }
  if (grupos.size) {
    const top = [...grupos.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(([donde, n]) => `${donde} (${n})`);
    sugerencias.push(
      `Con alguno de los términos sueltos (no todos) aparecen artículos en: ${top.join(', ')}, ` +
        'pero ninguno pasa tus filtros. Aflojá los filtros o cambiá los términos.',
    );
  } else {
    sugerencias.push(
      'Ningún término de la query aparece en el corpus. Probá con el vocabulario de la ' +
        'documentación, o empezá por `mapa()` para ver qué hay documentado.',
    );
  }

  // 3. Ramas del mapa relacionadas por término.
  const terminos = terminosDe(q);
  const ramas = idx.mapa
    .filter((rama) => {
      const texto = normalizarTermino(
        [
          rama.seccion,
          rama.categoria,
          rama.label,
          Array.isArray(rama.ejeValores) ? rama.ejeValores.join(' ') : '',
          Array.isArray(rama.modules) ? rama.modules.join(' ') : '',
        ]
          .filter(Boolean)
          .join(' '),
      );
      return terminos.some((t) => t.length > 2 && texto.includes(t));
    })
    .slice(0, 5);
  if (ramas.length) sugerencias.push('Ramas del mapa que mencionan tus términos: ver `ramasRelacionadas`.');

  return { sugerencias, ramasRelacionadas: ramas };
}

// ─────────────────────────────────────────────────────────────────── buscar

export function buscar({ q, page = 1, perfil = PERFIL.agente, ...resto } = {}) {
  const idx = indice();
  const politica = idx.eje;
  const filtros = normalizarFiltros(resto, politica);
  const pagina = Math.max(1, Math.floor(Number(page) || 1));

  // Los filtros duros (eje / seccion / dominio) son AND en los dos modos:
  // el fallback afloja la CONJUNCIÓN DE TÉRMINOS, nunca la metadata.
  const filtro = (r) => pasaFiltros(r, filtros, politica);

  let crudos = idx.mini.search(q, { combineWith: 'AND', filter: filtro });
  let modo = 'and';

  // Rescate de tipeo, sólo si el perfil lo pide y sólo como ÚLTIMO RECURSO.
  // El orden importa y es una decisión: va ANTES del relleno OR, porque ante un
  // error de tipeo "quisiste decir percepciones" le sirve más al lector que
  // "acá tenés todo lo que comparte alguna palabra". Prendido para TODA consulta
  // —y no sólo cuando la exacta da cero— los candidatos difusos compiten con las
  // coincidencias exactas y las empujan para abajo: eso se midió y rompía tres
  // casos del golden set.
  if (crudos.length === 0 && perfil.rescateDeTipeo) {
    crudos = idx.mini.search(q, { combineWith: 'AND', filter: filtro, fuzzy: 0.2 });
    if (crudos.length) modo = 'rescate-de-tipeo';
  }

  // Fix #12 de oba-docs — fallback automático a OR cuando el AND da 0. La
  // query la escribe un LLM que a veces pega la frase literal del cliente
  // ("quiero suscribirme al enterprise"): exigir que UN artículo contenga
  // TODOS los términos convierte eso en cero resultados, y devolver solo
  // hints obliga a un segundo roundtrip para llegar a la página que ya estaba
  // en el corpus. Preferimos devolver los artículos que matchean más
  // términos, rankeados, y decirle en la respuesta que verifique pertinencia.
  if (crudos.length === 0 && perfil.rellenoOr) {
    const porOr = idx.mini.search(q, { combineWith: 'OR', filter: filtro });
    if (porOr.length > 0) {
      crudos = porOr;
      modo = 'or-fallback';
    }
  }

  const total = crudos.length;
  const paginas = Math.max(1, Math.ceil(total / PAGINA_BUSCAR));
  const desde = (pagina - 1) * PAGINA_BUSCAR;
  // `fragmento`/`ancla`/`urlAncla` se calculan acá y no en `hitDe` por el
  // costo: recorren el cuerpo, y sólo se paga para los hits de esta página.
  const conCuerpo = idx.build.conCuerpo === true;
  const hits = crudos
    .slice(desde, desde + PAGINA_BUSCAR)
    .map((r) => ({ ...hitDe(r, politica), ...fragmentoDe(r, idx.porId.get(r.id), conCuerpo) }));

  const base = {
    buildId: idx.buildId,
    audiencia: idx.audiencia,
    q,
    filtros,
    // Qué capacidades tenía esta búsqueda. Reemplaza al viejo `fuzzy: FUZZY`,
    // que era un booleano global: con perfiles, el dato útil es cuál corrió.
    perfil: { ...perfil },
    // `and` = cada resultado contiene TODOS los términos. `or-fallback` = el
    // AND dio 0 y estos matchean ALGUNOS (ver `nota`).
    modo,
    total,
    page: pagina,
    paginas,
    hayMas: desde + hits.length < total,
    resultados: hits,
  };

  if (total === 0) return { ...base, ...hints(idx, q, filtros) };

  // De adhoc-docs, generalizado a los dos ejes: búsqueda sin filtro del eje
  // que trae resultados de varios valores — el agente se tiene que enterar
  // ANTES de redactar, no después. Mezclar en una misma respuesta la docu de
  // `oba` y la de `odumbo` (o la de la 18 y la 19) sin atribuir es el error
  // caro de este eje.
  if (politica.avisoMezcla && !filtros[politica.param]) {
    const presentes = [...new Set(hits.map((h) => h[politica.param]))].filter(Boolean).sort();
    if (presentes.length > 1) base.aviso = VOCABULARIO[politica.tipo].mezcla(presentes);
  }

  // El mismo aviso, para la otra dimensión. Mezclar el manual con las novedades
  // de versión sin atribuir es el error caro de un corpus con doc sets: el
  // agente lee una nota de cambio y la cita como el procedimiento vigente. Se
  // acumula al del eje en vez de pisarlo — los dos avisos son ciertos a la vez.
  if (politica.avisoMezclaDocSet && !filtros.docSet) {
    const presentes = [...new Set(hits.map((h) => h.docSet))].filter(Boolean).sort();
    if (presentes.length > 1) {
      const etiquetas = presentes
        .map((id) => politica.docSetsDeclarados.find((d) => d.id === id)?.label ?? id)
        .map((l) => `«${l}»`)
        .join(' y ');
      const aviso =
        `Estos resultados mezclan ${etiquetas}. Son documentaciones distintas: una dice cómo ` +
        'se hace algo hoy y la otra qué cambió respecto de la versión anterior. Atribuí cada ' +
        'resultado antes de citarlo, o filtrá con `docSet`.';
      base.aviso = base.aviso ? `${base.aviso} ${aviso}` : aviso;
    }
  }

  if (modo === 'or-fallback') {
    return {
      ...base,
      nota:
        'Ningún artículo contiene TODOS los términos de la query, así que se combinaron ' +
        'con OR: estos son los que matchean más términos, rankeados. Verificá pertinencia ' +
        'antes de citar — el primero puede no responder la pregunta. Los filtros por ' +
        'metadata siguen siendo exactos y duros.',
    };
  }

  // Fix de la 75201 — la ÚNICA señal de pobreza que se midió y separa algo.
  // Con un solo candidato no hay vecinos con los que comparar: el artículo
  // puede ser la respuesta, o puede ser el único que menciona esas palabras al
  // pasar. El motor no sabe cuál de las dos, y lo dice.
  //
  // «UN SOLO CANDIDATO» NO ES «UN SOLO ARTÍCULO DEL CORPUS», y la nota no lo
  // dice al revés a propósito. `total` sale de una búsqueda YA FILTRADA por
  // eje, sección y dominio, y la tool pide filtrar por eje siempre: el caso
  // normal es que el corpus tenga varios y el filtro deje uno. Prometer "el
  // corpus tiene uno" ahí es darle al agente una premisa falsa justo antes de
  // pedirle que conteste "no está documentado" — cuando la respuesta correcta
  // muchas veces es "está, en otra versión".
  //
  // LO QUE CUESTA: el 1,5% de las respuestas CORRECTAS cae acá (7 de 480
  // medidas sobre el corpus de oba-docs). Es el precio de declararlo, y es
  // barato porque la nota no esconde el resultado: sigue viniendo.
  //
  // LO QUE NO SE PUEDE, para que nadie lo reintente: no hay umbral que
  // detecte el match confiado y equivocado con 2+ resultados. Se midieron el
  // `score` crudo, la BRECHA entre el 1º y el 2º y la COBERTURA de los
  // términos en el título; las tres se superponen con las respuestas
  // correctas (la brecha suprime el 20,5% de las buenas en su corte más
  // bajo). Está medido en `scripts/golden/consultas.json` de oba-docs. La
  // razón es de fondo: las tres se calculan del match de palabras, y si el
  // artículo responde la pregunta es una cuestión de significado.
  //
  // POR QUÉ NO EN `or-fallback` Y SÍ EN `rescate-de-tipeo`: el primero ya trae
  // su propia nota. El segundo es evidencia MÁS débil que el `and` —la exacta
  // dio cero y esto es un rescate difuso—, así que un único candidato ahí
  // merece la misma advertencia, aunque el 1,5% se haya medido en `and`.
  //
  // Y ES UNA CAPACIDAD DEL PERFIL, no un `if` sobre el modo: este caso lo
  // alcanzan los dos llamadores, y la nota es prosa imperativa para un lector
  // que la lee. Al sitio no le sirve, y no la recibe.
  if (perfil.notaDePobreza && modo !== 'or-fallback' && total === 1) {
    return {
      ...base,
      nota:
        'De los artículos que pasan los filtros de esta búsqueda, sólo UNO contiene todos los ' +
        'términos. Con un único candidato no hay con qué compararlo: puede ser la respuesta, o ' +
        'puede ser el único que menciona esas palabras al pasar. Verificá contra su ' +
        '`description` y sus `headings` que responda la pregunta. Si no la responde, antes de ' +
        'contestar que la documentación no lo cubre probá aflojando los filtros o con otros ' +
        'términos — que acá quede uno no significa que el corpus tenga uno, y la respuesta ' +
        'puede estar bajo otro valor del filtro.',
    };
  }

  return base;
}

// ───────────────────────────────────────────────────────────────────── leer

/** Dice sobre bigramas: barato y suficiente para "¿quisiste decir?". */
function similitud(a, b) {
  const bigramas = (s) => {
    const n = normalizarTermino(s).replace(/[^a-z0-9]+/g, ' ');
    const out = new Set();
    for (let i = 0; i < n.length - 1; i++) out.add(n.slice(i, i + 2));
    return out;
  };
  const A = bigramas(a);
  const B = bigramas(b);
  if (!A.size || !B.size) return 0;
  let comunes = 0;
  for (const g of A) if (B.has(g)) comunes++;
  return (2 * comunes) / (A.size + B.size);
}

/** Ficha corta de un candidato, para las sugerencias. */
function fichaDe(articulo, politica) {
  const ficha = { slug: articulo.slug, title: articulo.title, url: absoluta(articulo.url) };
  if (politica.campo) ficha[politica.param] = articulo[politica.campo];
  return ficha;
}

/**
 * `leer()` — la política de desambiguación es UNA (diseno-eje.md §3):
 *
 *   1. cero candidatos           -> soft-fail `slug-inexistente` + sugerencias
 *   2. valor del eje pedido      -> match exacto || match fuera del eje
 *                                   || soft-fail `slug-fuera-del-valor-pedido`
 *   3. sin valor pedido:
 *      3a. un candidato          -> ese
 *      3b. hay uno fuera del eje -> ese (aplica a todos los valores)
 *      3c. `eje.default`         -> el del default, y se ANUNCIA (`elegidoPor`)
 *      3d. resto                 -> `encontrado: false`, `ambiguo-en-eje`
 *
 * `leer()` sólo elige cuando el config declaró a quién elegir, y cuando elige
 * lo dice. El fallback `candidatos[0]` —"el orden de inserción del índice",
 * exactamente lo que el comentario de oba-docs decía que había que evitar—
 * desaparece.
 */
export function leer({ slug, page = 1, ...resto } = {}) {
  const idx = indice();
  const politica = idx.eje;
  const { campo, param } = politica;
  const voc = campo ? VOCABULARIO[politica.tipo] : null;
  const pedido = param ? resto[param] : null;
  const pagina = Math.max(1, Math.floor(Number(page) || 1));
  const candidatos = idx.porSlug.get(slug) || [];

  const cabecera = { buildId: idx.buildId, audiencia: idx.audiencia };

  // Soft-fail: nunca un error seco. Un deploy que renombra un slug a mitad
  // de conversación no puede producir un falso "el artículo fue borrado".
  if (candidatos.length === 0) {
    const parecidos = idx.articulos
      .map((a) => ({
        ...fichaDe(a, politica),
        parecido: Math.max(similitud(slug, a.slug), similitud(slug, a.title || '')),
      }))
      .sort((a, b) => b.parecido - a.parecido)
      .slice(0, 5)
      .map(({ parecido, ...resto2 }) => ({ ...resto2, parecido: Math.round(parecido * 100) / 100 }));

    return {
      ...cabecera,
      encontrado: false,
      motivo: 'slug-inexistente',
      slugPedido: slug,
      mensaje:
        `No hay ningún artículo con el slug "${slug}" en el índice ${idx.buildId}. ` +
        'Esto NO significa que el artículo fue borrado: lo más probable es que el slug ' +
        'haya cambiado o esté mal escrito. Re-ejecutá `buscar()` con los términos del ' +
        'tema, o probá uno de los slugs sugeridos.',
      sugerencias: parecidos,
      siguientePaso: 'buscar({ q: "<términos del tema>" })',
    };
  }

  let articulo = null;
  let elegidoPor = null;

  if (campo && pedido != null) {
    // Fix #12 de oba-docs (mismo criterio que `pasaFiltros`): un artículo
    // fuera del eje (`eje: null`) es la respuesta correcta para cualquier
    // valor pedido. Sin esto, el flujo buscar() → leer() se rompía justo
    // para las páginas que ahora sí aparecen en los resultados: el hit venía
    // con `version: null` y el agente, que arrastra la versión de la
    // conversación, recibía un "no existe para esa versión".
    articulo =
      candidatos.find((a) => String(a[campo]) === String(pedido)) ||
      (politica.comodin ? candidatos.find((a) => a[campo] == null) : null) ||
      null;
    if (!articulo) {
      return {
        ...cabecera,
        encontrado: false,
        motivo: 'slug-fuera-del-valor-pedido',
        slugPedido: slug,
        [voc.pedido]: String(pedido),
        mensaje: voc.noCoincide(slug, pedido),
        [voc.disponibles]: candidatos.map((a) => a[campo]),
        sugerencias: candidatos.map((a) => fichaDe(a, politica)),
        siguientePaso: `leer({ slug: "${slug}", ${param}: "${candidatos[0][campo]}" })`,
      };
    }
  } else if (campo) {
    // 3a / 3b: un solo candidato, o uno fuera del eje (que aplica a todos los
    // valores y por lo tanto no es ambiguo).
    // Se llamaba `fueraDelEje`, igual que el campo de config que ya no existe.
    // Nunca fue eso: es el artículo que el COMODÍN del motor devuelve, y el
    // comodín es una capacidad del eje `version`, no algo que se declare.
    const bajoElComodin = politica.comodin ? candidatos.find((a) => a[campo] == null) : null;
    if (candidatos.length === 1) {
      articulo = candidatos[0];
    } else if (bajoElComodin) {
      articulo = bajoElComodin;
      elegidoPor = 'fuera-del-eje';
    } else if (politica.sinValor === 'default') {
      // 3c: manda el `default` del config, no el orden de inserción del
      // índice — que un reordenamiento silencioso de docs.config.json
      // volvería "la más vieja" sin que nadie lo note. Y se ANUNCIA: si la
      // plataforma se permite elegir, tiene que decir que eligió.
      articulo = candidatos.find((a) => String(a[campo]) === String(politica.default)) || null;
      if (articulo) elegidoPor = 'default';
    }
    if (!articulo) {
      // 3d: ambigüedad estructurada. Nunca `candidatos[0]`.
      const valores = candidatos.map((a) => a[campo]);
      return {
        ...cabecera,
        encontrado: false,
        motivo: 'ambiguo-en-eje',
        slugPedido: slug,
        mensaje: voc.ambiguo(slug, candidatos.length, valores),
        [voc.disponibles]: valores,
        sugerencias: candidatos.map((a) => fichaDe(a, politica)),
        siguientePaso: `leer({ slug: "${slug}", ${param}: "${candidatos[0][campo]}" })`,
      };
    }
  } else {
    // Sin eje los slugs son únicos: hay un solo candidato por definición.
    articulo = candidatos[0];
  }

  const body = articulo.body || '';
  const paginas = Math.max(1, Math.ceil(body.length / PAGINA_LEER));
  const desde = (pagina - 1) * PAGINA_LEER;
  const trozo = body.slice(desde, desde + PAGINA_LEER);

  const salida = {
    ...cabecera,
    encontrado: true,
    slug: articulo.slug,
    title: articulo.title,
    description: articulo.description,
    seccion: articulo.seccion,
    keywords: articulo.keywords || [],
    url: absoluta(articulo.url),
    headings: articulo.headings || [],
    page: pagina,
    paginas,
    hayMas: desde + trozo.length < body.length,
    body: trozo,
  };

  if (campo) {
    salida[param] = articulo[campo];
    salida[`${param}Label`] = idx.labelDeEje(articulo[campo]);
    // El mismo slug en otros valores del eje. Con eje `project` NO son "otras
    // versiones de lo mismo": son documentos distintos que casualmente
    // comparten nombre de archivo. La semántica la da `eje.tipo`, que viaja en
    // la cabecera del índice, así que el NOMBRE del campo es uno solo.
    salida.otrosDelEje = candidatos
      .filter((a) => a !== articulo)
      .map((a) => ({ valor: a[campo], url: absoluta(a.url) }));
    if (elegidoPor) {
      salida.elegidoPor = elegidoPor;
      if (elegidoPor === 'default') salida.mensaje = voc.elegido(articulo[campo]);
    }
  }
  for (const dominio of politica.camposDominio) {
    salida[dominio] = articulo[dominio] ?? (dominio === 'modules' ? [] : null);
  }

  return salida;
}

// ───────────────────────────────────────────────────────────────────── mapa

/**
 * Las secciones cuyo contenido cae bajo el COMODÍN del motor: artículos sin
 * valor de eje, que pasan cualquier filtro y ganan cualquier desambiguación.
 * Vacío si el corpus no tiene comodín (todo eje que no sea `version`) o si no
 * hay un solo artículo así.
 *
 * Se mide sobre los ARTÍCULOS y no sobre `mapa[].ejeValores`, aunque el
 * agregado parezca más barato. El campo `eje` del artículo es contrato del
 * MOTOR —es el que filtra `buscar()` y el que desambigua `leer()`, acá abajo—,
 * mientras que `mapa` lo arma el build de cada consumidor y los tres lo arman
 * distinto: oba mete el `null` en el Set, odumbo lo filtra con un `if` y
 * encima sólo emite el campo cuando el corpus es versionado, y adhoc escribe
 * `ejeValores: [a.eje]` con otra semántica. Derivar de ahí sería cambiar una
 * declaración que se pudre por un agregado que ya divergió.
 *
 * Para qué existe: que la prosa que anuncia el comodín la encienda el
 * CONTENIDO y no una declaración del config. Es el mismo criterio con el que
 * `politicaDeEje` deriva `paises` de `metadata` y no de una lista escrita a
 * mano — una tool que ofrece un filtro sin contenido detrás miente.
 */
export function seccionesConComodin() {
  const idx = indice();
  const politica = politicaDeEje(idx.build);
  if (!politica.comodin || !politica.campo) return [];
  const secciones = new Set();
  for (const a of idx.articulos) {
    if (a[politica.campo] == null && a.seccion) secciones.add(String(a.seccion));
  }
  return [...secciones].sort();
}

/**
 * `mapa()` — el nivel 1 del progressive disclosure: qué secciones y categorías
 * hay y cuántos artículos tiene cada una.
 *
 * `mapa({ seccion })` es el nivel 1,5: la lista de artículos de UNA sección,
 * para el agente que ya sabe dónde mirar y quiere ver qué hay sin adivinar
 * términos de búsqueda ("¿qué recursos hay?"). Sin `seccion` la respuesta es
 * idéntica a la de siempre, y hay un test que la compara contra la de v0.16.0.
 *
 * Con `seccion`, `articulos` deja de ser el total del corpus y pasa a ser la
 * lista: el total es de todo el índice y acá no dice nada. El árbol `mapa`
 * no viaja — para eso está la llamada sin parámetro.
 *
 * EL ORDEN ES EL DEL ÍNDICE, que es el que emite el build. En tuqui-docs el
 * emisor recorre `docs.json` en orden de navegación, así que es el orden del
 * sidebar; un build que emita en otro orden devuelve ese otro orden. El motor
 * no lo reordena porque no tiene de dónde sacar uno mejor.
 *
 * `categoria` sale del artículo si el build la emite, y si no del slug
 * (`seccion/categoria/…/pagina`): es la misma regla con la que los builds de
 * oba-docs y tuqui-docs arman los nodos del mapa, así que las dos coinciden.
 *
 * PAGINADO como `buscar()`: `PAGINA_MAPA` artículos por página, `page` desde 1,
 * con `total`, `page`, `paginas` y `hayMas`. Una sección grande entera pesa
 * más de lo que conviene meter de una vez en el contexto del agente.
 *
 * CON EJE, un solo valor: el pedido (`version` / `project`) o, si no se pide,
 * el `default` del corpus —el mismo que elige `leer()`, y se anuncia igual con
 * `elegidoPor: 'default'`—. Sin eso, cada slug aparecería una vez por valor.
 * Con eje `version` entran también los artículos fuera del eje, como en el
 * filtro de `buscar()`. Un eje sin `default` y sin valor pedido lista todos.
 *
 * SECCIÓN INEXISTENTE: soft-fail con las secciones válidas en `sugerencias`,
 * las más parecidas primero — el mismo criterio que el `slug-inexistente` de
 * `leer()`: nunca un error seco, siempre por dónde seguir.
 */
export function mapa({ seccion, page = 1, ...resto } = {}) {
  const idx = indice();
  const cabecera = {
    schemaVersion: idx.schemaVersion,
    buildId: idx.buildId,
    audiencia: idx.audiencia,
    generadoEl: idx.build.generatedAt,
  };
  if (seccion == null) {
    return {
      ...cabecera,
      articulos: idx.build.articulos ?? idx.articulos.length,
      metadata: idx.build.metadata ?? {},
      ...ejeEnMapa(idx),
      mapa: idx.mapa,
    };
  }

  const politica = idx.eje;
  const pedida = String(seccion);
  const pagina = Math.max(1, Math.floor(Number(page) || 1));
  const salida = { ...cabecera, metadata: idx.build.metadata ?? {}, ...ejeEnMapa(idx), seccion: pedida };

  const pedido = politica.param ? resto[politica.param] : null;
  const valor = pedido != null ? String(pedido) : politica.default;
  const delValor = (a) =>
    valor == null || String(a[politica.campo]) === valor || (politica.comodin && a[politica.campo] == null);
  if (politica.hay && valor != null) {
    salida[politica.param] = valor;
    if (pedido == null) salida.elegidoPor = 'default';
  }

  const enSeccion = idx.articulos.filter((a) => a.seccion === pedida && (!politica.hay || delValor(a)));
  const total = enSeccion.length;
  const paginas = Math.max(1, Math.ceil(total / PAGINA_MAPA));
  const desde = (pagina - 1) * PAGINA_MAPA;
  const articulos = enSeccion
    .slice(desde, desde + PAGINA_MAPA)
    .map((a) => {
      const ficha = {
        slug: a.slug,
        title: a.title,
        description: a.description,
        categoria: a.categoria ?? String(a.slug).split('/').slice(1, -1).join('/'),
        url: absoluta(a.url),
      };
      // Con eje, cada artículo dice su valor: `null` si está fuera del eje, o
      // cualquiera si el corpus no declara `default` y no se pidió uno.
      if (politica.campo) ficha[politica.param] = a[politica.campo];
      return ficha;
    });
  const paginado = { total, page: pagina, paginas, hayMas: desde + articulos.length < total };
  if (total) return { ...salida, ...paginado, articulos };

  const validas = [...new Set(idx.articulos.filter((a) => !politica.hay || delValor(a)).map((a) => a.seccion).filter(Boolean))]
    .map((s) => ({ s, parecido: similitud(pedida, s) }))
    .sort((a, b) => b.parecido - a.parecido || a.s.localeCompare(b.s))
    .map(({ s }) => s);
  const enValor = politica.hay && valor != null ? ` para ${politica.param} "${valor}"` : '';
  return {
    ...salida,
    ...paginado,
    articulos: [],
    motivo: 'seccion-inexistente',
    mensaje:
      `No hay ninguna sección "${pedida}"${enValor} en el índice ${idx.buildId}. Las que existen van en ` +
      '`sugerencias`, las más parecidas primero. Repetí `mapa()` con una de esas, o sin ' +
      '`seccion` para ver el árbol entero.',
    sugerencias: validas,
  };
}
