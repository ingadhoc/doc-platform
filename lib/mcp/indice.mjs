 ...
 *                en adhoc-docs). No son ejes: un artículo pertenece a UN
 *                valor del eje y a N valores de una faceta.
 */
export function politicaDeEje(build) {
  ...
  return {
    ...
    filtrosDominio: conModules ? ['modules'] : [],
    camposDominio: [...(conModules ? ['modules'] : []), ...(conTypes ? ['type'] : [])],
  };
}
...
function pasaFiltros(articulo, filtros, politica) {
  ...
  for (const dominio of politica.filtrosDominio) {
    ...
    const lista = Array.isArray(propios) ? propios : [propios];
