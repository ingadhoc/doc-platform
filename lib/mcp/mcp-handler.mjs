 ...
 *   · `cross` (artículos que aplican a todos los valores del eje) = sólo con
 *     eje `version`, que es donde el comodín está encendido; el texto nombra
 *     las secciones de `config.secciones.fueraDelEje` si están declaradas.
 *
 ...
 *   secciones         { fueraDelEje?: string[] } de `docs.config.json`: sólo
 *                     para nombrar esas secciones en la prosa de `buscar()`.
