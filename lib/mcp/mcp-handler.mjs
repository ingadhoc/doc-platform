    ...
    server.registerTool(
      'mapa',
      {
        ...
        description:
          ...
          (DOC_SETS
            ? `, más las documentaciones del corpus (${DOC_SETS.map((d) => `${d.id} = ${d.label}`).join(' · ')})`
            : '') +
    ...
    server.registerTool(
      'buscar',
      {
        ...
        description:
          ...
          // La contracara del or-fallback, y el fix de la 75201. El motor NO
          // puede detectar el match confiado y equivocado: se midieron el
          // score, la brecha entre el 1º y el 2º y la cobertura del título, y
          // ninguna separa (`scripts/golden/consultas.json` de oba-docs lo
          // tiene medido). Como no hay señal que emitir, la obligación viaja
          // acá, en el contrato, en vez de en una nota en cada respuesta: una
          // advertencia en todas las respuestas es la que nadie lee, y además
          // le sacaría el contraste a la nota del `or-fallback`, que hoy sí
          // significa algo.
          'Y al revés: que vuelva con `modo: "and"` quiere decir que hay artículos con ' +
          'TODOS los términos, no que respondan la pregunta. El match es léxico, así que ' +
          'un artículo puede tener todas las palabras y ser sobre otra cosa. Antes de citar ' +
          'el primero, verificá contra su `description` y sus `headings` que responda lo que ' +
          'se preguntó. Si ninguno de los que ves lo responde, mirá `hayMas` y `paginas` antes ' +
          'de concluir: la respuesta puede estar en una página que todavía no pediste, o bajo ' +
          'otro valor de los filtros. Recién ahí contestá que la documentación no lo cubre — ' +
          'es una respuesta correcta, y es mejor que citar el más parecido. ' +
          (E.hay
            ? E.duro +
              ' ' +
              (E.cross
