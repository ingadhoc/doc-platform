...
/**
 * 401 sin challenge, para un `fetch` del buscador o un asset. NUNCA un challenge
 * `Basic`: abriría el prompt del browser pidiendo una contraseña que ya no
...
export function crearGate(config = {}) {
  ...
  return async function decidir(request, env = process.env) {
    ...
    // ── Camino humano: la sesión firmada, y nada más ────────────────────────
    //
    // Se entra con el usuario de Odoo (task 72391). No hay credencial
    // compartida: `DOCS_AUTH_PASSWORD` y el challenge `Basic` se fueron en
    // v0.6.0, y con ellos las dos cosas que la credencial no podía dar — saber
    // quién entró, y que rotarla alcanzara (cada deployment viejo seguía
    // accesible con el secreto de su época).
    //
    // NO HAY BREAK-GLASS, y es a propósito: un segundo camino que abre todo es
    // exactamente lo que se vino a sacar. Si Odoo no responde, la documentación
    // interna no se sirve a humanos. Las máquinas no dependen de esto: el MCP y
    // los estáticos con Bearer siguen andando con `DOCS_MCP_TOKENS`.
    //
    // Fail-closed sobre la variable, igual que la audiencia.
    const secretoSesion = env.DOCS_SESION_SECRET;
    if (!secretoSesion) return malConfigurado('falta DOCS_SESION_SECRET');

    const sesion = await verificarSesion(leerCookie(request.headers.get('cookie')), secretoSesion);
    if (sesion) return null;

    // Todavía nadie: a la persona se la manda a loguearse, a las máquinas 401.
    return esNavegacion(request) ? aLogin(request) : pedirSesion();
  };
}
