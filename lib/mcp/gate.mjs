...
import { leerCookie, verificarSesion } from './sesion.mjs';
...
/**
 * LAS TRES RUTAS QUE EL GATE DEJA PASAR SIN CREDENCIAL. Son la puerta, no el
 * contenido: ninguna sirve documentación. Existen porque una puerta cerrada con
 * llave desde afuera no se abre nunca — sin esto, quien no tiene sesión no puede
 * llegar al login para conseguirla.
 *
 * `logout` está acá por lo mismo dado vuelta: quien tiene la sesión vencida
 * igual tiene que poder soltarla, y pedirle credencial para irse no tiene
 * sentido. Solo borra la cookie de quien la pide.
 *
 * Es una excepción a la regla 1 del middleware, así que es una LISTA EXACTA y no
 * un prefijo: con `/api/auth/*` alcanzaría agregar un archivo en esa carpeta para
 * publicar cualquier cosa sin gate, y nadie lo vería en el diff.
 *
 * Lo que se sirve en estas rutas se reparte SIN CREDENCIAL, así que nada de lo
 * que devuelvan puede valer como sesión: ver `PROPOSITO_INTENTO` en `sesion.mjs`.
 */
export const RUTAS_DE_PUERTA = ['/api/auth/login', '/api/auth/callback', '/api/auth/logout'];
...
/**
 * 401 sin challenge, para un `fetch` del buscador o un asset. Un challenge
 * `Basic` acá abriría el prompt del browser pidiendo una contraseña que en un
 * sitio ya migrado no existe.
 */
function pedirSesion() {
  return new Response('Sesión ausente o vencida. Entrá de nuevo a la documentación.', {
    status: 401,
    headers: {
      'Content-Type': 'text/plain; charset=utf-8',
      'Cache-Control': 'no-store',
...
/**
 * Manda a la persona a loguearse, guardando a dónde quería ir: quien abre un
 * artículo desde Slack tiene que terminar EN ese artículo, no en la home. El
 * `volver` lo valida la función de login, que es quien redirige de verdad.
 */
function aLogin(request) {
  const url = new URL(request.url);
  const volver = `${url.pathname}${url.search}`;
  return new Response(null, {
    status: 302,
    headers: {
      Location: `${RUTAS_DE_PUERTA[0]}?volver=${encodeURIComponent(volver)}`,
      'Cache-Control': 'no-store',
      'X-Robots-Tag': 'noindex, nofollow',
    },
  });
...
/**
 * ¿Es alguien navegando o una máquina pidiendo un archivo? Solo a la persona se
 * la manda al login; a un `fetch` se le contesta 401. Se mira `Accept`, que dice
 * lo que el cliente espera recibir, y no el `User-Agent`, que es adivinar.
 */
...
export function crearGate(config = {}) {
  ...
  return async function decidir(request, env = process.env) {
    ...
    // La puerta pasa sin credencial: es la única forma de llegar al login. Va
    // DESPUÉS del chequeo de audiencia a propósito — un build mal configurado
    // devuelve 503 hasta en la puerta. Ver `RUTAS_DE_PUERTA`.
    if (RUTAS_DE_PUERTA.includes(ruta)) return null;

    const esMcp = ruta === rutaMcp;
    ...
    // ── Camino humano: dos formas de ser alguien ────────────────────────────
    //
    // La SESIÓN FIRMADA (`DOCS_SESION_SECRET`) dice quién entró (task 72391). La
    // CREDENCIAL COMPARTIDA (`DOCS_AUTH_PASSWORD`) es el camino viejo, sin
    // trazabilidad, y mientras siga configurada es el break-glass para el día
    // que Odoo no responda: apagarla es borrar la variable, no tocar código.
    //
    // Fail-closed sobre las variables, igual que la audiencia: sin ninguna de
    // las dos no se sirve nada.
    const secretoSesion = env.DOCS_SESION_SECRET;
    const esperada = env.DOCS_AUTH_PASSWORD;
    if (!secretoSesion && !esperada) return malConfigurado('falta DOCS_AUTH_PASSWORD');

    if (secretoSesion) {
      const sesion = await verificarSesion(
        leerCookie(request.headers.get('cookie')),
        secretoSesion,
      );
      if (sesion) return null;
    }

    // La credencial compartida, mientras siga configurada. Quien mandó `Basic`
    // pidió este camino: si la credencial está mal, se le pide de nuevo y no se
    // lo manda al login — el prompt del browser ya está abierto delante suyo.
    if (esperada && header?.startsWith('Basic ')) {
      let usuario, clave;
      try {
      ...
      return usuarioOk && claveOk ? null : pedirCredencial(realm);
    }

    // Todavía nadie. Con login por Odoo configurado, a la persona se la manda a
    // loguearse y a las máquinas se les contesta 401. Sin login por Odoo, queda
    // el challenge Basic de siempre.
    if (secretoSesion) return esNavegacion(request) ? aLogin(request) : pedirSesion();
    return pedirCredencial(realm);
  };
}
