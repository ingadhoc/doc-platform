...
function coherencia(config) {
  ...
  // Doc sets: las dos referencias cruzadas que el JSON Schema no expresa.
  const docSets = Array.isArray(config.docSets) ? config.docSets : [];
  if (docSets.length) {
    const idsDocSet = docSets.map((d) => d?.id).filter(Boolean);
    const idsRep = idsDocSet.filter((id, i) => idsDocSet.indexOf(id) !== i);
    if (idsRep.length) {
      errores.push(`docSets: ids repetidos (${[...new Set(idsRep)].join(', ')})`);
    }
    // Una sección en dos doc sets es el error que rompe la unicidad del slug:
    // el índice lo detectaría, pero recién al construirse y con el corpus
    // entero emitido. Acá cuesta una línea y falla al leer el contrato.
    const vistas = new Map();
    for (const d of docSets) {
      for (const sec of d?.secciones ?? []) {
        if (vistas.has(sec)) {
          errores.push(
            `docSets: la sección "${sec}" está en dos doc sets ("${vistas.get(sec)}" y "${d.id}"). ` +
              'Un artículo pertenece a un solo doc set: si una sección está en dos, sus slugs ' +
              'quedan en los dos y el índice pisa un artículo con otro.',
          );
        } else {
          vistas.set(sec, d.id);
        }
    ...
    for (const d of docSets) {
      // Un `entrada` que no es una de sus secciones es un item del selector que
      // linkea a una ruta inexistente: sin error de config y sin error de build
      // hasta que alguien hace clic.
      if (d?.entrada && !(d.secciones ?? []).includes(d.entrada)) {
        errores.push(
          `docSets.${d.id}.entrada: "${d.entrada}" no es una de sus secciones ` +
            `(${(d.secciones ?? []).join(', ') || 'ninguna'}). La entrada es a dónde lleva el ` +
            'selector al elegir esta documentación.',
        );
      }
    }
    const conDefault = docSets.filter((d) => d?.default === true);
    if (conDefault.length > 1) {
      errores.push(
        `docSets: más de uno marcado \`default\` (${conDefault.map((d) => d.id).join(', ')}). ` +
          'El default es a cuál caer cuando la ruta no cae en ninguna sección conocida: ' +
...
/** La versión de este paquete: quién está validando. */
...
/** El tag que el repo consumidor pinea, leído de su `package.json`. */
function tagPineado(cwd) {
  try {
    const pkg = JSON.parse(readFileSync(resolve(cwd, 'package.json'), 'utf8'));
    const spec =
      pkg.dependencies?.['@ingadhoc/docs-platform'] ?? pkg.devDependencies?.['@ingadhoc/docs-platform'];
    return spec?.includes('#') ? spec.split('#').pop() : null;
  } catch {
    return null;
  }
}
...
/**
 * La pista que separa "este config está mal" de "tu instalación quedó vieja".
 * Un `node_modules` anterior al bump del pin rechaza una clave que la versión
 * pineada sí admite, y el mensaje —que no decía con qué versión validaba— se
 * lee como un config inválido. Barato de calcular y sólo se arma al fallar.
 */
function pistaDeVersion(cwd) {
  const instalada = versionInstalada();
  if (!instalada) return '';
  const linea = `\nValidado con @ingadhoc/docs-platform ${instalada}.`;
  const tag = tagPineado(cwd);
  if (!tag || tag.replace(/^v/, '') === instalada) return linea;
  return (
    `${linea} Este repo pinea ${tag}: tu instalación quedó desalineada del lock. ` +
    'Corré `npm ci` antes de mirar el config.'
  );
}
