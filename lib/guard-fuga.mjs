...
export function correrGuard({
  ...
  if (sondas.length) {
    ...
    //
    // DONDE EL CORTE NO LLEGA, y hay que saberlo: un índice de búsqueda no
    // serializa el HTML, serializa TEXTO CONDENSADO —cada bloque junto con UN
    // espacio, los blancos colapsados—, así que cuando ese JSON llega acá los
    // tags que marcaban el corte ya no existen y el cruce es indistinguible de
    // una frase. Medido sobre un `search-index.json` publicado: un párrafo que
    // cierra con un link cuyo texto termina en «…las facturas A», seguido de un
    // bloque que arranca «Esta página cubre…», pare ` a esta página `.
    // NO se arregla acá: recuperar el corte de un texto ya condensado es
    // adivinar, y toda forma de adivinarlo le saca detección al guard. Lo
    // cierra el `publicado` del preprocesador del consumidor, que hoy calcula
    // los trigramas públicos sobre el MARKDOWN —donde el destino del link
    // (`…-facturas-a.md`) mete palabras que el HTML no tiene y corre el
    // n-grama—, así que ese cruce nunca entra como público y la sonda sobrevive.
    //
